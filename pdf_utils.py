...
class MyPDFHandler(object):
    ...
    def save2file(self, new_file_name):
        ...
        print(f'save2file success! new file is: {new_file_name}')
    ...
    def add_one_bookmark(self, title, page, parent=None, color=None, fit='/Fit'):
        ...
        title.encode('utf-8')
        bookmarkRef = self.__writeable_pdf.addBookmark(title, page - 1, parent=parent, color=color, fit=fit)
        print(f'add_one_bookmark success! bookmark title is: {title}')
        return bookmarkRef
    ...
    def add_bookmarks(self, bookmarks):
        ...
        for title, page, level in bookmarks:
            if level == 1:
                parentStack.clear()
                parentStack.append(self.add_one_bookmark(title, page))
                continue

            while len(parentStack) > level - 1:
                parentStack.pop()
            parent = parentStack[-1]
            parentStack.append(self.add_one_bookmark(title, page, parent))
        print(f'add_bookmarks success! add {len(bookmarks)} pieces of bookmarks to PDF file')
    ...
    def read_bookmarks_from_txt(self, txt_file_path, page_offset=0):
        ...
        with open(txt_file_path, encoding='utf-8', mode='r') as f:
            for line in f:
                ...
                try:
                    level = len(line.split('@')[0].rstrip())
                    title = line.split('@')[1].strip()
                    page = line.split('@')[2].strip()
